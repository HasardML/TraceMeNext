...
| D008 | 2026-06-17 | `packingList` 从 `string[]` 升级为 `{ text: string, checked: boolean }[]`，并保留旧数据兼容迁移 | P24 需要行前清单支持勾选状态持久化，字符串数组无法表达交互状态；localStorage 旧数据读取和 JSON 导入归一化时将字符串转换为未勾选清单项。 | 继续使用 `string[]` 并另建 checked 状态映射 |
